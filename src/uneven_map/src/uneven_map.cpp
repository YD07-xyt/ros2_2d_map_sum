...
Eigen::Matrix3d UnevenMap::skewSym(Eigen::Vector3d vec) {
    ...
    skem_sym << 0.0, -vec(2), vec(1), vec(2), 0.0, -vec(0), -vec(1), vec(0), 0.0;
    return skem_sym;
}
...
double UnevenMap::calYawFromR(Eigen::Matrix3d R) {
    ...
    Eigen::Vector2d x =
        (Eigen::Matrix2d::Identity() + p * p.transpose() / (1.0 - p.squaredNorm())) * b;
...
void UnevenMap::init_params() {
    // --- 声明参数并设置默认值 ---
    nh_->declare_parameter<int>("iter_num", 0);
    nh_->declare_parameter<double>("map_size_x", 0.0);
    nh_->declare_parameter<double>("map_size_y", 0.0);
    nh_->declare_parameter<double>("ellipsoid_x", 0.0);
    nh_->declare_parameter<double>("ellipsoid_y", 0.0);
    nh_->declare_parameter<double>("ellipsoid_z", 0.0);
    nh_->declare_parameter<double>("xy_resolution", 0.0);
    nh_->declare_parameter<double>("yaw_resolution", 0.0);
    nh_->declare_parameter<double>("min_cnormal", 0.0);
    nh_->declare_parameter<double>("max_rho", 0.0);
    nh_->declare_parameter<double>("gravity", 9.81);
    nh_->declare_parameter<double>("mass", 1.0);
    nh_->declare_parameter<std::string>("map_pcd", "");
    nh_->declare_parameter<std::string>("map_file", "");

    // --- 获取参数值 ---
    nh_->get_parameter("iter_num", iter_num);
    nh_->get_parameter("map_size_x", map_size[0]);
    nh_->get_parameter("map_size_y", map_size[1]);
    nh_->get_parameter("ellipsoid_x", ellipsoid_x);
    nh_->get_parameter("ellipsoid_y", ellipsoid_y);
    nh_->get_parameter("ellipsoid_z", ellipsoid_z);
    nh_->get_parameter("xy_resolution", xy_resolution);
    nh_->get_parameter("yaw_resolution", yaw_resolution);
    nh_->get_parameter("min_cnormal", min_cnormal);
    nh_->get_parameter("max_rho", max_rho);
    nh_->get_parameter("gravity", gravity);
    nh_->get_parameter("mass", mass);
    nh_->get_parameter("map_pcd", pcd_file);
    nh_->get_parameter("map_file", map_file);
}
void UnevenMap::init(rclcpp::Node::SharedPtr nh) {
    this->nh_ = nh;
    init_params();
    origin_pub = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/origin_map", 10);
    filtered_pub = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/filtered_map", 1);
    zb_pub = nh->create_publisher<visualization_msgs::msg::Marker>("/zb_map", 10);
    so2_test_pub = nh->create_publisher<visualization_msgs::msg::MarkerArray>("/so2_map", 10);
    vis_timer = nh->create_wall_timer(std::chrono::milliseconds(100), [this]() { visCallback(); });
    ...
    for (int x = 0; x < voxel_num[0]; x++)
        for (int y = 0; y < voxel_num[1]; y++)
            for (int yaw = 0; yaw < voxel_num[2]; yaw++) {
                if (c_buffer[toAddress(x, y, yaw)] < min_cnormal ||
                    map_buffer[toAddress(x, y, yaw)].sigma > max_rho) {
...
bool UnevenMap::constructMapInput() {
    ...
    if (!pp.good()) {
        RCLCPP_WARN(nh_->get_logger(), "map file is empty, begin construct it.");
        return false;
    }
    ifstream fp;
    ...
    RCLCPP_INFO(nh_->get_logger(), "map: SO(2) --> RXS2 done.");
...
bool UnevenMap::constructMap() {
            ...
            for (int yaw = 0; yaw < voxel_num[2]; yaw++)
                for (int iter = 0; iter < iter_num; iter++) {
                    ...
                    if (iter == 0 && cnt++ % 100000 == 0) {
                        cout << "\033[1;33m map process "
                             << toAddress(x, y, yaw) * 100.0 /
                                    (voxel_num[0] * voxel_num[1] * voxel_num[2])
                             << "%\033[0m" << endl;
                        cnt = 1;
                    }
                }
    ...
    RCLCPP_INFO(nh_->get_logger(), "map: SE(2) --> RXS2 done.");
